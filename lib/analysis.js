...
class CodeAnalysis {
  constructor(options) {
    ...
    this.importQueue = []; // importItem分析插件
  ...
  _installPlugins(plugins) {
    ...
    /*********** */
    this.importQueue.push(defaultPlugin(this));
    // this.importQueue.push(methodPlugin(this));
  }
  ...
  _checkPropertyAccess(node, index = 0, apiName = "") {
    ...
    if (node.parent && tsCompiler.isPropertyAccessExpression(node.parent)) {
      index++;
  ...
  _runImportPlugins(
    ...
    if (this.importQueue.length > 0) {
      for (let i = 0; i < this.importQueue.length; i++) {
        const checkFun = this.importQueue[i].checkFun;
        if (
          checkFun(
            this,
            tsCompiler,
  ...
  _getFullCode(tsCompiler, node) {
    if (node.parent && !tsCompiler.isSourceFile(node.parent)) {
      // console.log("getFullCode", node.getFullText());
